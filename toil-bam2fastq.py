...
def runBAM2FASTQ(job, input_args, mystring):
    ...
    mycommand2 = "docker exec " + formatcontainerName + " sh -c 'chmod 666 /data/output_1.fastq'"
    os.system(mycommand2)

    mycommand2 = "docker exec " + formatcontainerName + " sh -c 'chmod 666 /data/output_2.fastq'"
    os.system(mycommand2)
    ...
    myzip_command = "pigz -c -p 30 " + os.path.join(work_dir, 'output_1.fastq') + " > " + os.path.join(work_dir, myfilename +  "_1.fastq.gz")
    os.system(myzip_command)

    myzip_command = "pigz -c -p 30 " + os.path.join(work_dir, 'output_2.fastq') + " > " + os.path.join(work_dir, myfilename +  "_2.fastq.gz")
    os.system(myzip_command)

    myzip_command = "pigz -c -p 30 " + os.path.join(work_dir, 'output_0.fastq') + " > " + os.path.join(work_dir, myfilename +  "_0.fastq.gz")
    os.system(myzip_command)
    ...
    s3am_command1= ['s3am', 'cancel', 's3://' + dest_bucket + '/sequence/' + myfilename + '_0.fastq.gz']

    s3am_command2 = ['s3am', 'upload', '--upload-slots=20', '--download-slots=20', 
                    '--part-size=50000000', 'file://{}'.format(os.path.join(work_dir, myfilename + '_0.fastq.gz')), 
                    's3://' + dest_bucket + '/sequence/nopair/' + myfilename + '_0.fastq.gz']
